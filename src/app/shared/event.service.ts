...
export class EventService {
  ...
  createEvent(event):Observable<IEvent> {
    let headers=new Headers({'Content-Type':'application/json'})
    let requestOptions=new RequestOptions({headers:headers})
    return this.http.post(this.url,requestOptions,event).pipe(map((response:Response)=>{
  ...
  updateEvent(event):Observable<IEvent> {
    let headers=new Headers({'Content-Type':'application/json'})
    let requestOptions=new RequestOptions({headers:headers})
    return this.http.put(this.url,requestOptions,event).pipe(map((response:Response)=>{
      return response.json()
    }))
  }
  ...
  searchSession(sessionKeyWord: string):Observable<ISession[]> {
